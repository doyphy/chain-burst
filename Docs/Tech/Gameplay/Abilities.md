...
- **방향**: 공격자 폰 위치 → 타격 지점(`ContextHandle`의 `HitResult`) → 자기 뒤쪽 순으로 폴백. 수평 성분만 쓴다.
  - **공격자가 1순위인 이유 — 타격 지점은 거리에 따라 의미가 변한다.** 멀리서 맞으면 "공격자 반대 방향"에 수렴하지만, 바짝 붙으면 몸통 반지름 수준의 **국소 표면 벡터**가 되어 공격자 위치와 사실상 무관해진다. 가로 스윙은 칼날이 몸통을 측면부터 훑고 들어오므로 첫 접촉점이 옆구리에 잡히고, **넉백이 측면으로 나간다.** 전진하는 공격 몽타주와 겹치면 플레이어와 몬스터가 서로 스쳐 지나가면서 공격이 계속 빗나간다.
    - 짧은 벡터를 정규화하면서 오차가 증폭되는 것까지 겹쳐, 가까울수록 방향이 튀는 게 구조적이다.
    - **혼합(가중치로 섞기)도 검토했으나 채택하지 않았다.** 방향별 피격 몽타주가 없어 플레이어에게 "옆구리를 맞았다"를 읽어줄 단서가 없고, 그 정도 편차는 넉백 거리·연쇄 피격·CMC 충돌이 만드는 변동에 묻힌다. **보이지 않는 효과를 위한 튜닝 값은 부채다.**
    - 피격 부위를 표현하고 싶으면 **방향별 피격 몽타주**(`SelectActionMontageIndex()`에서 타격 지점으로 앞/뒤/좌/우 선택)가 제자리다. 밀리는 위치가 아니라 애니메이션으로 읽히고, 타격 지점이 틀려도 **모션만 어색하지 위치가 어긋나지 않아** 실패 비용이 훨씬 싸다.
  - **타격 지점을 지우지는 않는다.** 공격자를 못 구하는 경우(환경 데미지·폭발 등)에는 "터진 지점 반대쪽"이 맞는 답이다. 폴백 자리에 두는 비용은 0이다.
  - 수평 투영은 후보를 만들 때마다 `GetSafeNormal2D()`로 함께 처리한다. **맨 마지막에 한 번만 투영하면**, 공격자가 수직으로 겹친 경우 3D 기준으로는 영벡터가 아니라 다음 후보를 건너뛰고 투영 후에야 0이 되어 **뒤쪽 폴백으로 빠진다.**
  ...
  - 넓은 호로 여럿을 벨 때 **부채꼴로 퍼지는 것도 여기서 따라온다.** 공격자 forward 기준이었다면 전원이 같은 방향으로 밀렸을 것이다.
  - 원거리 공격을 넣는다면 이 기준으로는 부족하다 — 발사체는 **시전자 위치가 아니라 발사체 진행 방향**이 맞는 힘의 방향이므로 별도 소스가 필요하다.
...
- **뒤에 다른 캐릭터가 있으면 안 밀린다.** CMC는 캐릭터끼리 밀지 않는다(`ApplyImpactPhysicsForces`가 물리 시뮬 바디에만 힘을 준다). 막히면 `SlideAlongSurface`로 옆으로 비껴가고, 정면이면 제자리다. 몬스터가 몰릴수록 밀림이 덜 보인다.
  - ⚠️ **"옆으로 빠진다"는 증상에는 원인이 둘 있었다.** 넉백 방향 자체가 측면으로 나가던 것(위 **방향** 항목에서 해소)과, 막힌 이동이 표면을 따라 미끄러지는 이 `SlideAlongSurface`다. 여기 남은 것은 후자뿐이며, 고치려면 공격 루트모션 중 측면 슬라이드를 억제하는 CMC 서브클래스가 필요하다(미도입).
