...
단일 몽타주와 콤보 몽타주를 구분해 저장하지 않고 엔트리 하나(태그 → 몽타주 배열)로 통합했음.
- 1개면 단일 액션, 여러 개면 콤보나 변형 풀이 되는데, 그 해석은 인덱스를 넘기는 쪽이 결정함.
...
서버가 몽타주를 직접 재생하지 않고 **인덱스를 큐로 전달**하기 때문에, 시뮬 프록시도 서버와 같은 클립을 재생함.
...
> 🔎 관련 문서 [Notion - 사망 몽타주 유지](https://sprout-whitefish-298.notion.site/v1-2-3cf2a74e9c6f80e9bfd4cb2f45666c04?pvs=143)