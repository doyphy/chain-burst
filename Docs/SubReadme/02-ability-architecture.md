...
노티파이를 통해 자신의 ASC 에게 태그 이벤트 전송 가능
...
- 종료 경로는 3가지이고, 어느 쪽으로 끝나든 정리 훅(`CleanupActionState()`)을 한 번 호출.
  - 정상 종료 — 애님 노티파이가 보낸 `Event_Action_EndAbility` 수신 → 몽타주 정지 + 상태 정리
  - 블렌드 아웃 종료 — 자기 몽타주 인스턴스의 블렌드 아웃 시작. 노티파이가 없거나 유실돼도 실제 재생(공격속도 반영)에 맞춰 종료. 인스턴스 ID로 기다리므로 같은 몽타주를 연속 재생해도 이전 재생과 섞이지 않음
  - 캔슬 종료 — 피격·사망 등으로 끊긴 경우. 서버에서만 몽타주 정지를 요청함 (다른 몽타주에 끊긴 경우는 이미 넘어갔으므로 정지하지 않음)
...
- 액션이 끝나면 콤보 체인을 닫고, `EndAbility` 에서 트레이스를 한 번 더 정지시켜 캔슬로 노티파이를 놓쳐도 트레이스 종료하게 함.

> 🔎 트레이스 감지 및 데미지 처리 구조는 [6 전투 시스템](06-combat-system.md) 참고
...
- 발동 시 `CancelAbilityTag` 로 진행 중인 어빌리티를 캔슬함 (예: 공격을 휘두르는 중 피격 → 공격만 끊고 피격 모션). 캔슬 판정은 어빌리티의 Asset Tags 와 비교하므로 몽타주 태그(`Action.*`)가 아니라 `Ability.*` 를 넣는다.
