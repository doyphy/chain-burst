...
- 플레이어의 입력을 감지해서 피벗 판단, 이동 잠금도 여기서 판단.
  - 감지는 여기서 하지만 판정 기준(속도·각도 임계)과 잠금 시간은 개이트별 이동 데이터에서 옴 → [4.6 피벗](04-locomotion-system.md#46-피벗--급반전-처리)
- 카메라 기준 이동 방향을 계산해 이동에 적용하고, 회전 컴포넌트에도 넘김 (질주 시 회전 타겟으로 사용)